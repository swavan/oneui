...
class SwaVanMockEndpoints(QWidget):
    ...
    def __init__(self):
        ...
        loadUi("templates/endpoints.ui", self)
    ...
    def update_view(self):
        ...
        if SwaVanCache.get_selected_env():
            for row in EndpointService.load_by_parent(SwaVanCache.get_selected_env()):
                item = QListWidgetItem(row.url)
                item.setWhatsThis(row.id)
                icon = QIcon(f"assets/images/icons/{'sad' if not row.is_active else row.http_method.lower()}.ico")
    ...
    def eventFilter(self, source, event):
        if event.type() == QEvent.Type.ContextMenu and source is self.mock_endpoints:
            menu = QMenu()
            _copy = menu.addAction(QIcon("assets/images/icons/copy.ico"), "Copy")
            _toggle = menu.addAction(QIcon("assets/images/icons/toggle.ico"), "Toggle")
